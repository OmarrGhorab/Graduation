...
def build_user_behavior_summary(user_id: str, user_profile: Dict) -> str:
    subject_preferences = user_profile.get("SubjectPreferences", [])
    preview_interests = user_profile.get("PreviewInterests", [])
    cart_subjects = user_profile.get("CartSubjects", [])
    user_interests = user_profile.get("UserInterests", [])
    completion_tendency = user_profile.get("CompletionTendency", "UNKNOWN")
    avg_completion_pct = user_profile.get("AvgCompletionPct", 0)
    avg_session_duration = user_profile.get("AvgSessionDuration", 0)
...
def build_user_numeric_features(user_profile: Dict) -> Dict[str, float]:
    all_analytics = user_profile.get("AllAnalytics", [])
    ...
    return {
        ...
        "cart_subjects_count": float(len(user_profile.get("CartSubjects", []))),
